...
theme.Product = (function() {
  ...
  Product.prototype = _.assignIn({}, Product.prototype, {
    ...
    _updateAddToCart: function(evt) {
      ...
      var cartItemsNum=0;
var disc=$('#discountBar');
var discountBar=$('#discountBar p');
var totalCartValue=0;
$('.mini-products-list .priceRow').each(function()
                                        {
 totalCartValue += (parseFloat($(this).find('.product-price').text().replace('$', '')) * parseFloat($(this).find('.qtyField span').text()));
cartItemsNum+=parseFloat($(this).find('.qtyField span').text());
                    });
...
if(cartItemsNum >= 8)
  {
    totalDiscount=totalCartValue*0.35;
    
    discountBar.text("Yay! You've saved %35 ($" + totalDiscount.toFixed(2) + ")" );
  }
else if(cartItemsNum >= 6)
  {
    totalDiscount=totalCartValue*0.30;
    discountBar.text("Yay! You've saved %30 ($" + totalDiscount.toFixed(2) + ")" );
  }
else if(cartItemsNum >= 4)
  {
    totalDiscount=totalCartValue*0.20;
    discountBar.text("Yay! You've saved %20 ($" + totalDiscount.toFixed(2) + ")" );
  }

console.log(totalCartValue.toFixed(2));
console.log(totalCartValue*0.35);
    },
...
$('.swatch.option1 .swatch-element .swatchLbl').click(function(){
  $('.swatch.option2 .swatch-element:not(.soldout):nth-of-type(1) input').click();
   var variantType=$(this).text().toLowerCase();
  console.log("work");
